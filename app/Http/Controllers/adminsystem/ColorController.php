...
class ColorController extends Controller {
  	...
  	public function loadData(Request $request){
        ...
        $data=DB::table('color')                  
                ->select('color.id','color.fullname','color.code','color.sort_order','color.status','color.created_at','color.updated_at')                
                ->where('color.fullname','like','%'.trim(mb_strtolower($filter_search,'UTF-8')).'%')                     
                ->groupBy('color.id','color.fullname','color.code','color.sort_order','color.status','color.created_at','color.updated_at')   
                ->orderBy('color.sort_order', 'asc')                
                ->get()->toArray();              
        $data=convertToArray($data);    
        $data=colorConverter($data,$this->_controller);            
        return $data;
    } 
    public function getForm($task,$id=""){     
      ...
      if(in_array($requestControllerAction, $arrPrivilege)){
        switch ($task) {
          case 'edit':
            ...
            $arrRowData=ColorModel::find((int)@$id)->toArray();                     
            break;
          case 'add':
     ...
     public function save(Request $request){
          ...
          if(empty($fullname)){
              ...
              $data=array();
              if (empty($id)) {
                $data=ColorModel::whereRaw("trim(lower(fullname)) = ?",[trim(mb_strtolower($fullname,'UTF-8'))])->get()->toArray();	        	
              }else{
                $data=ColorModel::whereRaw("trim(lower(fullname)) = ? and id != ?",[trim(mb_strtolower($fullname,'UTF-8')),(int)@$id])->get()->toArray();		
              }  
              if (count($data) > 0) {
                  $checked = 0;
                  $error["fullname"]["type_msg"] = "has-error";
          ...
          if(empty($code)){
                ...
                if (count($data) > 0) {
                  $checked = 0;
                  $error["code"]["type_msg"] = "has-error";
                  $error["code"]["msg"] = "Mã màu đã tồn tại";
                }       
            }                          
          if(empty($sort_order)){
             $checked = 0;
      ...
      public function trash(Request $request){
            ...
            if($checked == 1){                
                  ...
                  $sql = "DELETE FROM `color` WHERE `id` IN  (".$strID.")";                                                 
                  DB::statement($sql);                                    
            }
            $data                   =   $this->loadData($request);
            $info = array(
      ...
      public function sortOrder(Request $request){
            ...
            if(count($data_order) > 0){              
              foreach($data_order as $key => $value){      
                if(!empty($value)){
                  $item=ColorModel::find((int)@$value->id);                
      ...
      public function createAlias(Request $request){
          ...
          if (empty($id)) {              
              $dataColor=ColorModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();             
            }else{
              $dataColor=ColorModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
            }  
