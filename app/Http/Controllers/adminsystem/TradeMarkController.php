...
class TradeMarkController extends Controller {
  	var $_controller="trademark";	
  	var $_title="Thương hiệu";
  	...
  	public function loadData(Request $request){
        ...
        $data=DB::table('trademark')                  
                ->select('trademark.id','trademark.fullname','trademark.sort_order','trademark.status','trademark.created_at','trademark.updated_at')                
                ->where('trademark.fullname','like','%'.trim(mb_strtolower($filter_search,'UTF-8')).'%')                     
                ->groupBy('trademark.id','trademark.fullname','trademark.sort_order','trademark.status','trademark.created_at','trademark.updated_at')   
                ->orderBy('trademark.sort_order', 'asc')                
                ->get()->toArray();              
        $data=convertToArray($data);    
        $data=tradeMarkConverter($data,$this->_controller);            
        return $data;
    } 
    public function getForm($task,$id=""){     
      ...
      if(in_array($requestControllerAction, $arrPrivilege)){
        switch ($task) {
          case 'edit':
            ...
            $arrRowData=TradeMarkModel::find((int)@$id)->toArray();                     
            break;
          case 'add':
     ...
     public function save(Request $request){
          ...
          if(empty($fullname)){
              ...
              $data=array();
              if (empty($id)) {
                $data=TradeMarkModel::whereRaw("trim(lower(fullname)) = ?",[trim(mb_strtolower($fullname,'UTF-8'))])->get()->toArray();	        	
              }else{
                $data=TradeMarkModel::whereRaw("trim(lower(fullname)) = ? and id != ?",[trim(mb_strtolower($fullname,'UTF-8')),(int)@$id])->get()->toArray();		
              }  
              if (count($data) > 0) {
                  $checked = 0;
                  $error["fullname"]["type_msg"] = "has-error";
          ...
          if ($checked == 1) {    
                if(empty($id)){
                    $item         =   new TradeMarkModel;       
                    $item->created_at   = date("Y-m-d H:i:s",time());        
                      
                } else{
                    $item       = TradeMarkModel::find((int)@$id);   
      ...
      public function updateStatus(Request $request){
          ...
          if($checked==1){
              foreach ($arrID as $key => $value) {
                if(!empty($value)){
                    $item=TradeMarkModel::find($value);
                    $item->status=$status;
                    $item->save();      
                }            
              }
      ...
      public function trash(Request $request){
            ...
            if($checked == 1){                
                  ...
                  $sql = "DELETE FROM `trademark` WHERE `id` IN  (".$strID.")";                                                 
                  DB::statement($sql);                                    
            }
            $data                   =   $this->loadData($request);
            $info = array(
      ...
      public function sortOrder(Request $request){
            ...
            if(count($data_order) > 0){              
              foreach($data_order as $key => $value){      
                if(!empty($value)){
                  $item=TradeMarkModel::find((int)@$value->id);                
      ...
      public function createAlias(Request $request){
          ...
          if (empty($id)) {              
              $dataTradeMark=TradeMarkModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();             
            }else{
              $dataTradeMark=TradeMarkModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
            }  
