...
use App\SizeStandardModel;
use DB;
class SizeStandardController extends Controller {
  	var $_controller="size_standard";	
  	var $_title="Kích thước";
  	...
  	public function loadData(Request $request){
        ...
        $data=DB::table('size_standard')                  
                ->select('size_standard.id','size_standard.fullname','size_standard.sort_order','size_standard.status','size_standard.created_at','size_standard.updated_at')                
                ->where('size_standard.fullname','like','%'.trim(mb_strtolower($filter_search,'UTF-8')).'%')                     
                ->groupBy('size_standard.id','size_standard.fullname','size_standard.sort_order','size_standard.status','size_standard.created_at','size_standard.updated_at')   
                ->orderBy('size_standard.sort_order', 'asc')                
                ->get()->toArray();              
        $data=convertToArray($data);    
        $data=originConverter($data,$this->_controller);            
        return $data;
    } 
    public function getForm($task,$id=""){     
      ...
      if(in_array($requestControllerAction, $arrPrivilege)){
        switch ($task) {
          case 'edit':
            ...
            $arrRowData=SizeStandardModel::find((int)@$id)->toArray();                     
            break;
          case 'add':
     ...
     public function save(Request $request){
          ...
          if(empty($fullname)){
              ...
              $data=array();
              if (empty($id)) {
                $data=SizeStandardModel::whereRaw("trim(lower(fullname)) = ?",[trim(mb_strtolower($fullname,'UTF-8'))])->get()->toArray();	        	
              }else{
                $data=SizeStandardModel::whereRaw("trim(lower(fullname)) = ? and id != ?",[trim(mb_strtolower($fullname,'UTF-8')),(int)@$id])->get()->toArray();		
              }  
              if (count($data) > 0) {
                  $checked = 0;
                  $error["fullname"]["type_msg"] = "has-error";
          ...
          if ($checked == 1) {    
                if(empty($id)){
                    $item         =   new SizeStandardModel;       
                    $item->created_at   = date("Y-m-d H:i:s",time());        
                      
                } else{
                    $item       = SizeStandardModel::find((int)@$id);   
      ...
      public function updateStatus(Request $request){
          ...
          if($checked==1){
              foreach ($arrID as $key => $value) {
                if(!empty($value)){
                    $item=SizeStandardModel::find($value);
                    $item->status=$status;
                    $item->save();      
                }            
              }
      ...
      public function trash(Request $request){
            ...
            if($checked == 1){                
                  ...
                  $sql = "DELETE FROM `size_standard` WHERE `id` IN  (".$strID.")";                                                 
                  DB::statement($sql);                                    
            }
            $data                   =   $this->loadData($request);
            $info = array(
      ...
      public function sortOrder(Request $request){
            ...
            if(count($data_order) > 0){              
              foreach($data_order as $key => $value){      
                if(!empty($value)){
                  $item=SizeStandardModel::find((int)@$value->id);                
      ...
      public function createAlias(Request $request){
          ...
          if (empty($id)) {              
              $dataOrigin=SizeStandardModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();             
            }else{
              $dataOrigin=SizeStandardModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
            }  
