...
class OriginController extends Controller {
  	...
  	public function loadData(Request $request){
        ...
        $data=DB::table('origin')                  
                ->select('origin.id','origin.fullname','origin.sort_order','origin.status','origin.created_at','origin.updated_at')                
                ->where('origin.fullname','like','%'.trim(mb_strtolower($filter_search,'UTF-8')).'%')                     
                ->groupBy('origin.id','origin.fullname','origin.sort_order','origin.status','origin.created_at','origin.updated_at')   
                ->orderBy('origin.sort_order', 'asc')                
                ->get()->toArray();              
    ...
    public function getForm($task,$id=""){     
      ...
      if(in_array($requestControllerAction, $arrPrivilege)){
        switch ($task) {
          case 'edit':
            ...
            $arrRowData=OriginModel::find((int)@$id)->toArray();                     
            break;
          case 'add':
     ...
     public function save(Request $request){
          ...
          if(empty($fullname)){
              ...
              $data=array();
              if (empty($id)) {
                $data=OriginModel::whereRaw("trim(lower(fullname)) = ?",[trim(mb_strtolower($fullname,'UTF-8'))])->get()->toArray();	        	
              }else{
                $data=OriginModel::whereRaw("trim(lower(fullname)) = ? and id != ?",[trim(mb_strtolower($fullname,'UTF-8')),(int)@$id])->get()->toArray();		
              }  
              if (count($data) > 0) {
                  $checked = 0;
                  $error["fullname"]["type_msg"] = "has-error";
          ...
          if ($checked == 1) {    
                if(empty($id)){
                    $item         =   new OriginModel;       
                    $item->created_at   = date("Y-m-d H:i:s",time());        
                      
                } else{
                    $item       = OriginModel::find((int)@$id);   
      ...
      public function trash(Request $request){
            ...
            if($checked == 1){                
                  ...
                  $sql = "DELETE FROM `origin` WHERE `id` IN  (".$strID.")";                                                 
                  DB::statement($sql);                                    
            }
            $data                   =   $this->loadData($request);
            $info = array(
      ...
      public function sortOrder(Request $request){
            ...
            if(count($data_order) > 0){              
              foreach($data_order as $key => $value){      
                if(!empty($value)){
                  $item=OriginModel::find((int)@$value->id);                
      ...
      public function createAlias(Request $request){
          ...
          if (empty($id)) {              
              $dataOrigin=OriginModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();             
            }else{
              $dataOrigin=OriginModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
            }  
