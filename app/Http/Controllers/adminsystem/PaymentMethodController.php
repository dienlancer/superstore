...
switch ($task) {
  ...
  public function createAlias(Request $request){
        ...
        if (empty($id)) {
          $dataPaymentMethod=PaymentMethodModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();            
        }else{
          $dataPaymentMethod=PaymentMethodModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
        }          
        if (count($dataPaymentMethod) > 0) {
