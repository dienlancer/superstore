...
class UnitController extends Controller {
  	...
  	public function loadData(Request $request){
        ...
        $data=DB::table('unit')                  
                ->select('unit.id','unit.fullname','unit.sort_order','unit.status','unit.created_at','unit.updated_at')                
                ->where('unit.fullname','like','%'.trim(mb_strtolower($filter_search,'UTF-8')).'%')                     
                ->groupBy('unit.id','unit.fullname','unit.sort_order','unit.status','unit.created_at','unit.updated_at')   
                ->orderBy('unit.sort_order', 'asc')                
                ->get()->toArray();              
        $data=convertToArray($data);    
        $data=unitConverter($data,$this->_controller);            
        return $data;
    } 
    public function getForm($task,$id=""){     
      ...
      if(in_array($requestControllerAction, $arrPrivilege)){
        switch ($task) {
          case 'edit':
            ...
            $arrRowData=UnitModel::find((int)@$id)->toArray();                     
            break;
          case 'add':
     ...
     public function save(Request $request){
          ...
          if(empty($fullname)){
              ...
              $data=array();
              if (empty($id)) {
                $data=UnitModel::whereRaw("trim(lower(fullname)) = ?",[trim(mb_strtolower($fullname,'UTF-8'))])->get()->toArray();	        	
              }else{
                $data=UnitModel::whereRaw("trim(lower(fullname)) = ? and id != ?",[trim(mb_strtolower($fullname,'UTF-8')),(int)@$id])->get()->toArray();		
              }  
              if (count($data) > 0) {
                  $checked = 0;
                  $error["fullname"]["type_msg"] = "has-error";
      ...
      public function trash(Request $request){
            ...
            if($checked == 1){                
                  ...
                  $sql = "DELETE FROM `unit` WHERE `id` IN  (".$strID.")";                                                 
                  DB::statement($sql);                                    
            }
            $data                   =   $this->loadData($request);
            $info = array(
      ...
      public function sortOrder(Request $request){
            ...
            if(count($data_order) > 0){              
              foreach($data_order as $key => $value){      
                if(!empty($value)){
                  $item=UnitModel::find((int)@$value->id);                
      ...
      public function createAlias(Request $request){
          ...
          if (empty($id)) {              
              $dataUnit=UnitModel::whereRaw("trim(lower(alias)) = ?",[trim(mb_strtolower($alias,'UTF-8'))])->get()->toArray();             
            }else{
              $dataUnit=UnitModel::whereRaw("trim(lower(alias)) = ? and id != ?",[trim(mb_strtolower($alias,'UTF-8')),(int)@$id])->get()->toArray();    
            }  
